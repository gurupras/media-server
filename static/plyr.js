typeof navigator === "object" && (function (global, factory) {
  ...
  var defaults = {
    ...
    // 'rewind',
    'play', // 'fast-forward',
    'progress', 'current-time', 'mute', 'volume', 'captions', 'settings', 'pip', 'airplay', 'googlecast', 'fullscreen'],
  ...
  var googleCast = {
    setup: function setup(config) {
      ...
      googleCast.debug = new Console(true); // TODO: Get cast logs under a separate namespace?
      // Inject the container
      ...
      if (!window.chrome.cast) {
        loadScript(this.config.urls.googleCast.api).then(function () {
          ...
          var interval = setInterval(function () {
            if (window.chrome.cast.isAvailable) {
              ...
              var opts = extend({}, googleCast.defaults, config);
    ...
    initializeCastApi: function initializeCastApi(config) {
      var framework = window.cast.framework;
      var CastContext = framework.CastContext;
      CastContext.getInstance().setOptions(config.options); // Set up event handlers
    ...
    getCurrentPlyr: function getCurrentPlyr() {
      return googleCast.currentPlyr;
    },
    onPlay: function onPlay() {
      var plyr = googleCast.getCurrentPlyr();
      googleCast.debug.log('Asking remote player to play'); // Seek before playing?
      // googleCast.onSeek();
    ...
    onPause: function onPause() {
      ...
      plyr.remotePlayerController.playOrPause(); // Seek after pause

      googleCast.onSeek();
    },
    onSeek: function onSeek() {
      var plyr = googleCast.getCurrentPlyr();
      var timestamp = plyr.currentTime;
      plyr.remotePlayer.currentTime = timestamp;
      plyr.remotePlayerController.seek();
      googleCast.debug.log("Asking remote player to seek to ".concat(timestamp));
    },
    ...
    onVolumeChange: function onVolumeChange() {
      var plyr = googleCast.getCurrentPlyr(); // We need to specially handle the case where plyr is muted

      var volume = plyr.volume;
    ...
    onQualityChange: function onQualityChange() {
      var plyr = googleCast.getCurrentPlyr();
      googleCast.loadMedia(plyr);
    },
    loadMedia: function loadMedia(plyr, loadRequest) {
      ...
      session.loadMedia(loadRequest).then(function () {
        ...
        googleCast.debug.log("Error during loadMedia: ".concat(err));
      });
    },
    ...
    bindPlyr: function bindPlyr(plyr, options) {
      ...
      googleCast.currentPlyrOptions = options; // TODO: Figure out if we should do plyr.remotePlayer = plyr.remotePlayer || new window.cast.framework.RemotePlayer()

      plyr.remotePlayer = new window.cast.framework.RemotePlayer(); // TODO: Figure out if we should do plyr.remotePlayerController = plyr.remotePlayerController || new window.cast.framework.RemotePlayerController(plyr.remotePlayer);
    ...
    unbindPlyr: function unbindPlyr(plyr) {
      var currentPlyr = googleCast.currentPlyr;
    ...
    getErrorMessage: function getErrorMessage(error) {
      ...
      switch (error.code) {
        case chrome.cast.ErrorCode.API_NOT_INITIALIZED:
          return "The API is not initialized.".concat(error.description ? " :".concat(error.description) : '');

        case chrome.cast.ErrorCode.CANCEL:
          return "The operation was canceled by the user".concat(error.description ? " :".concat(error.description) : '');

        case chrome.cast.ErrorCode.CHANNEL_ERROR:
          return "A channel to the receiver is not available.".concat(error.description ? " :".concat(error.description) : '');

        case chrome.cast.ErrorCode.EXTENSION_MISSING:
          return "The Cast extension is not available.".concat(error.description ? " :".concat(error.description) : '');

        case chrome.cast.ErrorCode.INVALID_PARAMETER:
          return "The parameters to the operation were not valid.".concat(error.description ? " :".concat(error.description) : '');

        case chrome.cast.ErrorCode.RECEIVER_UNAVAILABLE:
          return "No receiver was compatible with the session request.".concat(error.description ? " :".concat(error.description) : '');

        case chrome.cast.ErrorCode.SESSION_ERROR:
          return "A session could not be created, or a session was invalid.".concat(error.description ? " :".concat(error.description) : '');

        case chrome.cast.ErrorCode.TIMEOUT:
          return "The operation timed out.".concat(error.description ? " :".concat(error.description) : '');

        default:
          return "Unknown error: ".concat(JSON.stringify(error));
      }
    },
    castStateListener: function castStateListener(data) {
      googleCast.debug.log("Cast State Changed: ".concat(JSON.stringify(data)));
      var plyr = googleCast.getCurrentPlyr();
      var cs = window.cast.framework.CastState;
      var castEvent;
      ...
      if (plyr && castEvent) {
        var castActive = castEvent === 'castenabled'; // Add class hook
    ...
    sessionStateListener: function sessionStateListener(data) {
      var plyr = googleCast.getCurrentPlyr();
      ...
      googleCast.debug.log("sessionStateListener: state=".concat(data.sessionState));
    },
    requestSession: function requestSession(plyr) {
      ...
      var existingPlyr = googleCast.getCurrentPlyr();
      ...
      function onRequestSuccess(e) {// This only triggers when a new session is created.
      ...
      if (session === null || wasPlyrAlreadyBound) {
        var promise = window.cast.framework.CastContext.getInstance().requestSession();
        ...
        var cs = window.cast.framework.CastContext.getInstance().getCastState();
        var castStateEventData = new window.cast.framework.CastStateEventData(cs);
        googleCast.castStateListener(castStateEventData);
        var ss = window.cast.framework.CastContext.getInstance().getSessionState();
        var sessionStateEventData = new window.cast.framework.SessionStateEventData(session, ss, 0);
    ...
    show: function show() {
      ...
      var active = this.storage.googlecast; // Otherwise fall back to the default config

      if (!is.boolean(active)) {
        active = this.googlecast.active;
      } else {
  ...
  var ui = {
    ...
    build: function build() {
      ...
      googleCast.setup.call(this); // Reset volume
  ...
  function () {
    ...
    _createClass(Listeners, [{
      ...
      value: function controls$$1() {
        ...
        bind(this.player.elements.buttons.googlecast, 'click', this.player.googleCast, 'googlecast'); // Fullscreen toggle
  ...
  var source = {
    ...
    change: function change(input) {
      ...
      this.destroy.call(this, function () {
        ...
        _this2.media = null; // Remove hls property if set

        delete plyr.hls; // Reset class name
